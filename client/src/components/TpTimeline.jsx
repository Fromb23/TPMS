...
const TpTimeline = ({ currentPhase, documentStatus }) => {
  ...
  const tpTimeline = [
    ...
    },
    {
      id: 2,
      name: 'Verification',
      status: documentStatus?.status === 'PENDING' ? 'current' :
    ...
    },
    {
      id: 3,
      name: 'Active TP',
      status: documentStatus?.status === 'APPROVED' || ['active-tp', 'assessment'].includes(currentPhase) ? 'current' :
             ['document-submission', 'pre-tp'].includes(currentPhase) ? 'pending' : 'completed',
    },
    {
      id: 4,
      name: 'Assessments',
      status: currentPhase === 'assessment' ? 'current' :
             ['post-tp', 'completed'].includes(currentPhase) ? 'completed' : 'pending',
    },
    {
      id: 5,
      name: 'Final Submission',
      status: currentPhase === 'post-tp' ? 'current' :
